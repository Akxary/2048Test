...
from db_models.db_funcs import add_score, get_user_scores as get_db_user_scores, get_user_by_id
...
async def add_user_score(score: CreateScore, db: Session = Depends(get_db)):
    ...
    return GetScore(user_id=db_score.user_id, score=db_score.value, game_time=db_score.game_time)


@router.get("/{user_id}", response_model=GetScore)
